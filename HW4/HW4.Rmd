...
inspect(subset(topic_assoc, lift > 1))
plot(head(topic_assoc, 100, by='lift'))
...
#sm1 <- merge(sm1,apply(X = sm1,FUN = sum,MARGIN = 1),by=0) %>% column_to_rownames("Row.names") %>%
#  rename("Total" = "y")
...
sm_scl <- data.frame(sm_scl)
```

```{r eval=FALSE,echo=FALSE}

PCAsm <- prcomp(sm_scl,rank=5)
summary(PCAsm)
sm_x <- PCAsm$rotation
sm_x %>% data.frame() %>% arrange(desc(PC1))
PCAx <- PCAsm$x

sm_scl$Total = apply(X=sm_scl,FUN=sum,MARGIN = 1)
sm_scl$cluster <- clust1$cluster
sm_scl$PC1 <- PCAx[,1]
sm_scl$PC2 <- PCAx[,2]
sm_scl$PC3 <- PCAx[,3]

ggplot(data=sm_scl)+
geom_point(aes(cluster, fashion, color=Total),alpha=.4)
```
